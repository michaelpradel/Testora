...
def filter_prs_by_risk(github_prs):
    # split into chunks of 50
    chunks = [github_prs[i:i + 50] for i in range(0, len(github_prs), 50)]

    # ask LLM to rank PRs by risk of introducing a regression bug
    result = []
    for chunk in chunks:
        prompt = PRRegressionBugRanking(github_prs)
        raw_answer = llm.query(prompt)
